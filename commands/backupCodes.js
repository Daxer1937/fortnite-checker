...
async function handleViewBackupCodes(interaction, auth) {
  try {
    ...
    const embed = {
      ...
      color: 0x0099FF,
      fields: [],
      footer: { text: '⚠️ Keep these codes secure and private!' }
    };
...
async function handleGenerateBackupCode(interaction, auth) {
  try {
    ...
    const embed = {
      ...
      description: 'Keep this code secure! You can use it to recover your account.',
      color: 0x00FF00,
      fields: [
        {
          name: '🔐 Backup Code',
        ...
        {
          name: '⚠️ Important',
          value: 'Save this code in a secure location. It can be used to recover your Epic Games account.',
          inline: false
        }
      ],
      footer: { text: 'This code will only be shown once!' }
    };
...
async function handleDeleteBackupCode(interaction, auth, codeId) {
  try {
    ...
    await interaction.followUp({
      content: `✅ Backup code deleted successfully!`,
      ephemeral: true
    });
