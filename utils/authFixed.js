...
class EpicGamesAuth {
  constructor() {
    ...
    // Try multiple Epic Games endpoints
    this.endpoints = [
      'https://account-public-service-prod.ol.epicgames.com',
      'https://account-public-service.epicgames.com',
      'https://epicgames.com/account/api'
    ];
    this.epicApi = this.endpoints[0];
  ...
  async findWorkingEndpoint() {
    for (const endpoint of this.endpoints) {
      ...
      if (working) {
        this.epicApi = working;
        console.log(`✅ Using endpoint: ${working}`);
        return working;
      }
    }
    throw new Error('No working Epic Games API endpoint found');
  }

  async startDeviceFlow() {
    // First, find a working endpoint
    if (!this.epicApi || this.epicApi === this.endpoints[0]) {
